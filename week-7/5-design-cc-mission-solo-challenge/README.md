...
To see an example of one student's theme creativity, click [here](http://imears.github.io/super_starfish/SuperStarFish.html). There is no reason to make your "game" have a code combat feel, and there is also no reason to go this in depth unless you have the time. START with and MVP and start SMALL. This can be a very simple challenge so start that way first. If you have time, desire, and energy, THEN make it more complex and do the bonus release. No one is comparing your code to one another. The overall goal is simply to create objects and functions that interact. Period.
