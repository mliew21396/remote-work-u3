...
You should have the `congress_poll_results.db` in this challenge directory (It may not show up in the directory structure, but it should be visible in the terminal by typing `ls`. To access a database from within Ruby code, you need to install the sqlite3 gem.  Run `gem install sqlite3` from the command line. Open the database up from the command line by typing  `sqlite3 congress_poll_results.db` in the correct local directory. If you have trouble, try googling the error or ask your cohort-mates for help.
